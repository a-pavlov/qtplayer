...
VlcMedia::VlcMedia(const QString &location,
    ...
    : QObject(instance), vlcMediaMemoryStream(nullptr)
{
...
VlcMedia::VlcMedia(VlcMediaMemoryStream* ms
                   , VlcInstance* instance)
    : QObject(instance), vlcMediaMemoryStream(nullptr) {
...
VlcMedia::VlcMedia(libvlc_media_t *media): vlcMediaMemoryStream(nullptr)
{
...
int vlc_mem_seek(void *opaque, uint64_t offset) {
    VlcMediaMemoryStream* ms = static_cast<VlcMediaMemoryStream*>(opaque);
    Q_ASSERT(ms != nullptr);
...
void VlcMedia::initMedia(VlcMediaMemoryStream* ms,
